...
<head>
    ...
    <title>CarsBomb - Démander un retrait</title>
    ...
    <div class="container-fluid py-3" style="padding: 0; overflow-x: hidden;">
        ...
        <main style="margin: 0; padding: 0;">
            <div class="container">
                ...
                @if (Session::get('error_withdrawal') != null)
                <div class="alert alert-danger mt-4">
                    {{ Session::get('error_withdrawal') }}
                </div>
                @endif

                @if (Session::get('success_withdrawal') != null)
                <div class="alert alert-success mt-4">
                    {{ Session::get('success_withdrawal') }}
                </div>
                @endif
            </div>
            ...
            <div class="game-description" style="padding-bottom: 80px;">
                <h1 style="text-align: center;">Demander un retrait</h1>
                <form action="{{ route('public.withdrawal.store') }}" class="row" method="POST">
                    @csrf
                    <div class="col-md-6 offset-md-3">
                        <div style="margin-bottom: 10px;">
                            <label for="amount">Montant à rétirer (en €):</label>
                            <input type="number" name="amount" id="amount" class="form-control">
                        </div>
                        <div class="buttons">
                            <button class="btn btn-danger" type="submit">Valider la demande</button>
                        </div>
                    </div>
                </form>
            </div>
